...
contract('MusicianManager', accounts => {
    ...
    it('Should not add a track to an unknown artist', async () => {
        ...
        let err = null;
        try {
            await contract.addTrack('0xd3320e1bcc80bdad051caa32eccbbbcace4ec90f', 'Track01', 252, {from: accounts[1]})
        } catch (error) {
            err = error;
        }
        assert.ok(err instanceof Error);
    ...
    it('Should get the tracks of an artist', async () => {
        const contract = await MusicianManager.deployed();
        const result = await contract.getTracks('0x499e316e8c8dae95096d30b9fef3424c3bd8963c', {from: accounts[0]});
